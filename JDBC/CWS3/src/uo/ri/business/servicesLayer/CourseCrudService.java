...
public interface CourseCrudService {
	 ...
	 *        and the table of percentages. The id and the version fields must
	 *        be null (will be assigned by the system).
	 *
	 * @return the same Dto with the id field assigned to the created UUID
	 *
	 * @throws BusinessException, if: - any field other than id and version is
	 *                            null or empty, or - there already exists a
	 *                            course with the same name, or - there is
	 *                            percentage devoted to a non existing vehicle
	 *                            type, or - the initial and final dates are in
	 *                            the past or inverted, or - the number of hours
	 *                            are zero or negative, or - there are no
	 *                            dedications specified, or - the sum of devoted
	 *                            percentages does not equals 100%, or - the are
	 *                            any dedication with an invalid percentage
	 *                            (empty, zero, negative)
	 */
	 ...
	 *        match the current state of the course. All the rest of fields must
	 *        be filled, even if there is no change in the data. So it must pass
	 ...
	 *                           course does not match the version of the dto,
	 *                           or - the course has its start date in the past,
	 *                           or - the new data does not pass the validation
	 ...
	 *
	 * @param id
	 * @throws BusinessException if: - there is no course with the specified id,
